...
use crate::{MAX_STEM, domain::{model::SyntheticSS, process::compute_pages}, solve::action::Action, types::{Kind, Torsion}};
...
pub fn write_typescript_file(
    ...
    for (from,to) in &data.temp_fakes {
        ...
        if d_y == 0 {
            int_tau_mults.push(InternalTauMult { from: data.model.name(*from).to_string(), to: data.model.name(*to).to_string(), kind: Kind::Unknown, proof: Some(format!("Unknown")), page: 500 });
        } else if d_stem == 0 {
            ext_tau_mults.push(ExternalTauMult { from: data.model.name(*from).to_string(), to: data.model.name(*to).to_string(), kind: Kind::Unknown, proof: Some(format!("Unknown")) });
        } else {
            differentials.push(Differential { from:  data.model.name(*from).to_string(), to: data.model.name(*to).to_string(), kind: Kind::Unknown, proof: Some(format!("Unknown")) });
        }
    }
