...
use crate::{MAX_STEM, domain::model::{PagesGeneratorState, SSPages, SyntheticSS}, data::naming::name_get_tag, solve::{action::Action, issues::Issue}, types::Torsion};
...
pub fn auto_deduce(data: &SyntheticSS, issue: &Issue) -> Result<Action, ()> {
    match issue {
        Issue::SyntheticE1Page { stem, af, expected, observed } => {
            if expected.len() == 1 {
                debug_assert_eq!(observed.len(), 1);
                for id in data.model.gens_id_in_stem_af(*stem, *af) {
                    if data.model.y(*id) == 1 {
                        ...
                        return Ok(Action::SetE1 { 
                            ...
                            torsion: expected[0], 
                            proof: format!("Only one generator in stem {stem}, af {af}")
                        });
                    }
                }
            }
        ...
        Issue::InvalidTorsion { from, to, to_needed } => Err(()),
        Issue::SyntheticConvergence { bot_trunc, top_trunc, stem, af, expected, observed } => Err(()),
        Issue::InvalidAEHP { from, to } => Err(()),
        Issue::AlgebraicConvergence { bot_trunc, top_trunc, stem, af, expected, observed } => Err(()),
    }
}
